import type { ServiceConfig } from "./types";

export const landschaftspflege: ServiceConfig = {
  slug: "landschaftspflege",
  breadcrumbLabel: "Baumarbeiten",
  leistungsbereichLabel: "Leistungsbereich 01",
  heroTitle: "Baumarbeiten.",
  heroEmphasis: "Professionell. Termintreu.",
  heroIntro:
    "Von der Kronenpflege bis zum Notfallservice –\nwir arbeiten fachgerecht, sicher und ohne Umwege.\nEin Ansprechpartner. Festpreis. Erledigt.",
  bigNumber: "01",

  leistungen: [
    {
      nummer: "01",
      titel: "Kronenpflege & -sicherung",
      text: "Totholzentnahme, Schnittmaßnahmen und Kronensicherungssysteme für standsichere, gesunde Bäume. Wir arbeiten nach aktuellen FLL-Richtlinien.",
    },
    {
      nummer: "02",
      titel: "Baumfällungen",
      text: "Fachgerechte Fällungen mit Seiltechnik, Maschinen oder Hebebühne – nach sorgfältiger Begutachtung und unter Einhaltung aller gesetzlichen Vorgaben.",
    },
    {
      nummer: "03",
      titel: "Kletterarbeiten",
      text: "Präzise Eingriffe direkt im Baum, auch in engen Gärten, Innenhöfen oder ohne Zufahrt für Großgeräte. Schonend für die Umgebung.",
    },
    {
      nummer: "04",
      titel: "Wurzelstockentfernung mit Stuppenfräse",
      text: "Vollständige Entfernung mit Stuppenfräse oder Baggertechnik. Flächen werden danach wieder nutzbar – für Pflanzungen, Rasen oder Pflasterarbeiten.",
    },
    {
      nummer: "05",
      titel: "Häckselarbeiten",
      text: "Professionelles Häckseln von Ästen, Schnittgut und Gehölzresten direkt vor Ort. Sauber, effizient und platzsparend entsorgt.",
    },
    {
      nummer: "06",
      titel: "Obstbaumschnitt",
      text: "Erziehungs-, Erhaltungs- oder Verjüngungsschnitt – individuell abgestimmt auf Baumart, Alter und Standort für maximale Gesundheit und Ertrag.",
    },
    {
      nummer: "07",
      titel: "Notfallservice bei Sturmschäden",
      text: "Schnelle Reaktion nach Unwettern. Wir beseitigen akute Gefahren durch umgestürzte Bäume oder instabile Kronenteile – zuverlässig und ohne Verzug.",
    },
  ],

  vorteile: [
    "Ausgebildete Baumkletterer mit Seiltechnik",
    "Moderne Maschinen & Hebebühne",
    "Einhaltung FLL-Richtlinien",
    "Notfallservice 24/7",
    "Festpreisgarantie",
    "Termintreu & verlässlich",
  ],

  warumHeadline: "Fachbetrieb.\nNicht Hobbyist.",
  warumText:
    "Ausgebildete Fachkräfte, moderner Maschinenpark und über 8 Jahre Erfahrung in Thüringen.\nKein Subunternehmer. Kein Versprechen ohne Deckung.",

  gallery: [
    "/images/services/landschaftspflege/bild1.jpg",
    "/images/services/landschaftspflege/bild2.jpg",
    "/images/services/landschaftspflege/bild3.jpg",
    "/images/services/landschaftspflege/bild4.jpg",
    "/images/services/landschaftspflege/bild5.jpg",
    "/images/services/landschaftspflege/bild6.jpg",
  ],
};